...
bool eth_direct_ring_gather_sender_receiver_kernels(
    ...
    for (uint32_t i = 0; i < sender_receivers.size(); ++i) {
        ...
        auto& sender_program = programs.try_emplace(sender_device->id(), tt::tt_metal::CreateScopedProgram()).first->second;
        auto& receiver_program = programs.try_emplace(receiver_device->id(), tt::tt_metal::CreateScopedProgram()).first->second;
