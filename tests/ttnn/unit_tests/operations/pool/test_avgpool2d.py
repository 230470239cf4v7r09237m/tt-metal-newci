...
def run_avg_pool(
    ...
    ttnn_output = ttnn.to_device(ttnn_output, device)
    ttnn_output = ttnn.permute(ttnn_output, (0, 3, 1, 2))  # N, C, H, W
    ttnn_output = torch.Tensor(ttnn.to_torch(ttnn_output))
...
def test_run_avg_pool(device, input_shape, kernel_size, stride, padding, dilation, shard_scheme, ceil_mode):
